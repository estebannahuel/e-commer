...
const CartPage = () => {
  ...
  const { isAuthenticated } = useAuth(); // Solo necesitamos saber si está autenticado
  ...
  const handleCheckout = () => {
    ...
    const order = placeOrder(cartItems, getCartTotal());
    if (order) {
      alert(`¡Compra realizada con éxito! Tu número de orden es: ${order.id}`);
              ...
              {cartItems.map(item => (
                <ListGroup.Item key={item.id} className="d-flex align-items-center py-3">
                  ...
                  <Form.Control
                    type="number"
                    min="1"
                    // Asumiendo que el item del carrito no tiene stock completo,
                    // sino que el stock se verifica al añadir. Aquí solo se asegura
                    // que la cantidad sea al menos 1.
                    value={item.quantity}
